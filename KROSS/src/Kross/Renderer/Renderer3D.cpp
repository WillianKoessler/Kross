...
namespace Kross {
	...
	struct PointBatchData {
		Ref<Shader> geometryShader, rasterShader;
	...
	void Renderer3D::Init()
	{
		...
		Stack<Shader>::instance().Add(pData->rasterShader = Shader::CreateRef("assets/shaders/OpenGL/voxelShader.glsl"));
		//Stack<Shader>::instance().Add(pData->geometryShader = Shader::CreateRef("assets/shaders/OpenGL/geometryShader.glsl"));
		//Stack<Shader>::instance().Add(pData->rasterShader = Shader::CreateRef("assets/shaders/OpenGL/rasterShader.glsl"));
	...
	void Renderer3D::Begin(Ref<Camera::Camera>& camera)
	{
		...
		pData->rasterShader->Bind();
		pData->rasterShader->SetMat4("u_ViewProjection", camera->GetVPM());
		pData->rasterShader->SetMat4("u_Transform", glm::mat4(1.0f));
	}
