...
namespace Kross::Camera3D {
	...
	FirstPerson::FirstPerson(const char* cname, float aspectRatio, float fov)
		: m_fAR(aspectRatio), m_fFOV(fov), mouseRotation(glm::vec3(0.0f, 0.0f, 1.0f))
	{
		if (camera) KROSS_CORE_WARN("WARNING: Overriding previous camera: {1}", camera->GetName());
		camera = makeRef<Perspective>(m_fAR, m_fFOV, cname);
		static const char *a = "_Controller";
		std::string strname(cname);
		strname += "_Controller";
		name = new char[strname.size()];
	...
	FirstPerson::FirstPerson(const char* name, Camera::Camera* cam, float aspectRatio, float fov)
		: m_fAR(aspectRatio), m_fFOV(fov), mouseRotation(glm::vec3(0.0f, 0.0f, 1.0f)), name(nullptr)
	{
		if (camera) KROSS_CORE_WARN("WARNING: Overriding previous {2} Camera: {1}", camera->GetName(), camera->GetType());
		camera.reset(cam);
		SetName(name);
		KROSS_CORE_INFO("'{0}' Constructed", GetName());
	}
	...
	void FirstPerson::CalculateView() { camera->SetProjMat(m_fFOV, m_fAR, 0.01f, 1000.0f); }
