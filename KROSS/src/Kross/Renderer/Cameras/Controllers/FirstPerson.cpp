...
namespace Kross::Camera3D {
	...
	void FirstPerson::OnUpdate(Timestep ts)
	{
		KROSS_PROFILE_FUNC();
	...
	void FirstPerson::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<MouseScrolledEvent>(KROSS_BIND_EVENT_FN(FirstPerson::OnMouseScrolled));
	...
	{
		m_fZoom = std::clamp<float>(val, 0.05f, 1000.0f);
		CalculateView();
	}
	...
	{
		KROSS_PROFILE_FUNC();
		float val = e.GetYOffSet() * m_fZoom * 0.01f + m_fZoom;
		Zoom(val);
		KROSS_CORE_TRACE("{0}: Zoom = {1}", __FUNCTION__, val);
		return false;
	}
	...
	bool FirstPerson::OnMouseMoved(MouseMovedEvent& e)
	{
		...
		static glm::vec2 last = glm::vec2(0);
		...
		if (mouseRotation.x > BOUND) mouseRotation.x = BOUND;
		else if (mouseRotation.x < -BOUND) mouseRotation.x = -BOUND;

		if (mouseRotation.y > 360) mouseRotation.y = 0;
		else if (mouseRotation.y < 0) mouseRotation.y = 360;

		last = glm::vec2(e.GetX(), e.GetY());
		return false;
	}
}