...
namespace Kross::OpenGL {
	...
	public:
		...
		virtual void DrawArrays(const Scope<VertexArray>& va, uint32_t indexCount, bool drawPoints) const override;
		virtual void DrawIndexed(const Ref<VertexArray>& va) const override;
		virtual void DrawIndexed(const Scope<VertexArray>& va, uint32_t indexCount, bool drawPoints) const override;
