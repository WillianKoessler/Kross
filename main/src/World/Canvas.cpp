...
static struct windows
{
	...
	bool keyboard = true; bool keyboardState = true;
	bool gamepad = true; bool gamepadState = true;
	bool viewports = false; bool viewportsState = false;
...
void Canvas::OnImGuiRender(Kross::Timestep ts)
{
	using namespace ImGui;
	{
		...
		if (!manager.run_applicacion) app.OnEvent(Kross::WindowCloseEvent());
		...
		{
			//manager.viewportsState = manager.viewports = false;
...
int Canvas::MainWindow(bool* p_open, const char* id)
{
	...
	manager.keyboard = io.ConfigFlags & ImGuiConfigFlags_NavEnableKeyboard;
	manager.gamepad = io.ConfigFlags & ImGuiConfigFlags_NavEnableGamepad;
	manager.viewports = io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable;
